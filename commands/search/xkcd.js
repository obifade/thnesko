...
bot.registerCommand('xkcd', (msg, args) => {
    ...
    axios.get(`http://xkcd.com/${idURL}info.0.json`).then((response) => {
        bot.createMessage(msg.channel.guild.id, `**${msg.author.username}**, ${response.data.title} \n ${response.data.alt} \n ${response.data.img}`).catch(error => log.errC(error));
    }).catch((response) => {
        if (response.response.status === 404) {
            log.errC(`rate limit hit: ${response}`);
            bot.createMessage(msg.channel.guild.id, `**${msg.author.username}**, I couldn't find a comic of that number. It is likely higher than the current.`).catch(error => log.errC(error));
        } else {
            log.errC(response);
